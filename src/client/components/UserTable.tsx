...
export const UserTable: React.FC<UserTableProps> = (props) => {
    ...
    const columnFilterFn: (arg0: string) => [string, boolean] = (col: string) => {
    ...
    const [
        displayedColumns,
        setDisplayedColumns
    ] = React.useState<[string, boolean][]>(allColumns.map(columnFilterFn));
    ...
    return <div className="userTableContainer">
        {
            nPages() > 1 ?
                <div className="controls">
                    <button className="prev" onClick={ incPage(-1) }>prev page</button>
                    <div className="pageNumber">page { page + 1 } of { nPages() }</div>
                    <button className="next" onClick={ incPage(1) }>next page</button>
                </div>
                : null
        }
