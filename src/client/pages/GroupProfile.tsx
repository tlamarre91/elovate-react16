...
import { AuthWall, PageTitle, GroupEditForm } from '~client/components';
...
export const GroupProfile: React.FC<GroupProfileProps> = (props) => {
    ...
    const mainProfile = !group ? null : (
        <>
            ...
            {loggedInUserIsAdmin ? (
                <div className="editGroupLink">
                    <Link to={`${url}/edit`}>Edit group</Link>
                </div>
            ) : null}
            {!group.publicVisible ? (
    ...
    return (
        <div className="groupProfile page">
            {!ready ? (
